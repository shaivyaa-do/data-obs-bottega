...
/**
  * Opens a PostgreSQL JDBC connection and runs `SELECT 1`. Password is passed
  * to [[DriverManager.getConnection]] as a parameter, never as a URL query
  * string, so it is not copied into the URL that exceptions might echo.
  */
trait JdbcSelectOne {
  def selectOne(url: String, username: String, password: String): Unit
}

trait JdbcListTables {
  def listTables(url: String, username: String, password: String, schema: String): Seq[String]
}

object PostgresJdbcTester extends JdbcSelectOne with JdbcListTables {
